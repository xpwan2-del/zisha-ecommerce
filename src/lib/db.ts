...
function resolveProjectRoot(): string {
  ...
  for (const start of candidates) {
    let dir = start;
    for (let i = 0; i < 12; i++) {
      const dbCandidate = path.join(dir, 'src/lib/db/database.sqlite');
      const wasmCandidate = path.join(dir, 'node_modules/sql.js/dist/sql-wasm.wasm');
      const gitDirCandidate = path.join(dir, '.git');
      const hasRealGitDir = fs.existsSync(gitDirCandidate) && fs.statSync(gitDirCandidate).isDirectory();
      if (fs.existsSync(dbCandidate) && fs.existsSync(wasmCandidate) && hasRealGitDir) {
        return dir;
      }
  ...
  for (const start of candidates) {
    ...
    for (let i = 0; i < 12; i++) {
      const dbCandidate = path.join(dir, 'src/lib/db/database.sqlite');
      const wasmCandidate = path.join(dir, 'node_modules/sql.js/dist/sql-wasm.wasm');
      if (fs.existsSync(dbCandidate) && fs.existsSync(wasmCandidate)) {
        return dir;
      }
      const parent = path.dirname(dir);
      if (parent === dir) break;
...
const dbPath = path.join(projectRoot, 'src/lib/db/database.sqlite');
...
async function initDB(): Promise<SqlJsDatabase> {
  ...
  // 加载SQL.js WASM模块
  const wasmBuffer = fs.readFileSync(path.join(projectRoot, 'node_modules/sql.js/dist/sql-wasm.wasm'));
