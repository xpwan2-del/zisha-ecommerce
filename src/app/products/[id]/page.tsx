...
export default function ProductDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleBuyNow = async () => {
    ...
    if (!isAuthenticated || !user) {
      router.push(`/login?redirect=/quick-order?product_id=${product.id}&quantity=${quantity}`);
      return;
    }
    ...
    try {
      ...
      if (!data.success) {
        setCartMessage({ type: 'error', text: i18n.language === 'ar' ? 'المخزون غير كافٍ، لا يمكن الشراء' : i18n.language === 'en' ? 'Insufficient stock, cannot purchase' : '库存不足，无法购买' });
        return;
      }

      router.push(`/quick-order?order_number=${data.data.order_number}`);
    } catch (error) {
      console.error('Buy now error:', error);
      setCartMessage({ type: 'error', text: i18n.language === 'ar' ? 'فشلت العملية، يرجى إعادة المحاولة' : i18n.language === 'en' ? 'Operation failed, please retry' : '操作失败，请重试' });
    }
  };
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFavorited) {
        await fetch(`/api/favorites?product_id=${product.id}`, { method: 'DELETE', credentials: 'include' });
        setIsFavorited(false);
      } else {
        await fetch('/api/favorites', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ product_id: product.id }) });
        setIsFavorited(true);
      }
    } catch (error) {
