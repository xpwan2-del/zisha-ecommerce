...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  logApiRequest('PRODUCTS', 'POST', '/api/admin/products/[id]/unpublish');
  ...
  try {
    ...
    await query("UPDATE products SET publish_status = 'unpublished', updated_at = CURRENT_TIMESTAMP WHERE id = ?", [productId]);
    ...
    return NextResponse.json({ success: true, data: { id: productId, publish_status: 'unpublished' } });
  } catch (error: any) {
    logApiError('PRODUCTS', 'POST', error);
    return createErrorResponse('PRODUCT_UNPUBLISH_FAILED', 500);
  }
}
