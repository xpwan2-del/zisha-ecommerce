...
export async function DELETE(
  ...
  try {
    ...
    const checkResult = await query(
      'SELECT id, check_number, status, total_products, operator_name FROM inventory_checks WHERE id = ?',
      [checkId]
    );
    ...
    await recordAdminAuditLog({
      ...
      module: 'INVENTORY',
      action: 'DELETE_INVENTORY_CHECK',
      description: `管理员删除库存盘点单: ${check.check_number}`,
      ...
      metadata: {
        ...
        originalOperator: check.operator_name
      }
    });
