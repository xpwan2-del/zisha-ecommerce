...
> 本文件是 XPSKILL.md 的完整副本 + 项目开发记忆
> XPSKILL.md 文件位置：`/Users/davis/zisha-ecommerce/.trae/skills/xpskill/XPSKILL.md`
> ⚠️ 不要修改 XPSKILL.md 文件！
...
需要新增的 API
API 说明 POST /api/cart/checkout/validate  结算前验证 POST /api/cart/checkout 创建订单
数据库修改
orders 表（已有）order_items 表（已有）需要确保事务支持
...
│                      （库存当前状态）                            │
│                                                                 │
│  字段：product_id, product_name, quantity,                       │
...
│                        （预警事件）                              │
│                                                                 │
│  字段：product_id, alert_type(code), current_stock,             │
│  threshold, status, ...                                        │
│                                                                 │
│  作用：记录触发预警的事件，像报警记录                             │
└─────────────────────────────────────────────────────────────────┘
...
│                     （变动类型字典表）                           │
│                                                                 │
│  字段：code, name, name_en, direction, description              │
│                                                                 │
│  作用：定义所有库存变动类型，不硬编码                             │
└─────────────────────────────────────────────────────────────────┘
                                │
...
│                      （单据类型字典表）                           │
│                                                                 │
│  字段：code, name, name_en, description                         │
│                                                                 │
│  作用：定义所有关联单据类型，不硬编码                             │
└─────────────────────────────────────────────────────────────────┘
...
核心原则
单一数据来源：inventory 表是库存的唯一真实来源
后端计算：所有库存状态、预警判断都在后端完成
前端渲染：前端只负责展示后端返回的数据
字典表管理：类型定义不硬编码，存数据库字典表
表结构汇总
...
#### 问题分析
- **核心问题**：FeaturedProducts.tsx 中 `fetchProducts()` 和 `fetchInventoryStatuses()` 两个并行请求导致竞态条件
- **表现**：当 fetchInventoryStatuses 还没返回时，products 已经处理完毕，导致库存状态显示不正确
- **根本原因**：前端试图自己计算库存状态，而不是依赖API返回的完整数据

#### 解决方案
采用**服务端逻辑优先**原则：
- 修改所有相关API，直接返回完整的 `stock_status_info` 对象
- 前端不再需要调用 `fetchInventoryStatuses`
- 移除了 FeaturedProducts.tsx 和 products/[id]/page.tsx 中的 `fetchInventoryStatuses` useEffect

#### 数据库变更
- 创建了 `inventory_status` 表，包含4种状态：
  1. 有货 (quantity > 10)
  2. 库存有限 (5 < quantity <= 10)
  3. 库存紧张 (1 <= quantity <= 5)
  4. 缺货 (quantity <= 0)
- 修改了 `inventory` 表：添加 `status_id` 外键，移除旧的 `status` 字段

#### API修改清单（9个文件）
1. `/api/home/route.ts` - 添加 inventory_status JOIN
2. `/api/promotions/route.ts` - 添加 inventory_status JOIN
3. `/api/products/deals/route.ts` - 添加 inventory_status JOIN
4. `/api/cart/route.ts` - 添加 inventory_status JOIN
5. `/api/inventory/checks/[id]/complete/route.ts` - 修复状态计算
6. `/api/inventory/route.ts` - 修复状态计算
7. `/api/inventory/init/route.ts` - 修复状态计算
8. `/api/products/seed/route.ts` - 修复状态计算
9. `/api/db/table/[table]/[id]/route.ts` - 添加自动 status_id 计算

#### 前端修改（2个文件）
1. `src/components/FeaturedProducts.tsx`：
   - 移除 `fetchInventoryStatuses` useEffect
   - 简化产品处理逻辑，直接使用 API 返回的 `stock_status_info`
   - 添加 `statusId=4`（缺货）标签显示
   - 移除硬编码的 `bestSeller` 逻辑（product.id <= 2）

2. `src/app/products/[id]/page.tsx`：
   - 移除 `fetchInventoryStatuses`
   - 直接使用 `product.stock_status_info`
   - 修复 `{{count}}` 翻译问题

### 2. 修复的问题
| 问题 | 修复方式 | 日期 |
|-----|---------|------|
| SQLITE_ERROR: no such column: i.status | 更新9个API使用 i.status_id | 2026-04-16 |
| Race condition | API直接返回完整stock_status_info | 2026-04-16 |
| Type mismatch in find() | 添加 Number() 转换 | 2026-04-16 |
| bestSeller hardcoded | 完全移除bestSeller逻辑 | 2026-04-16 |
| 缺货状态不显示标签 | 添加 statusId=4 处理 | 2026-04-16 |
| products表已无stock字段 | 更新产品API改为读写inventory表 | 2026-04-16 |
| 商品列表排序用到p.stock | 改为 ORDER BY i.quantity DESC | 2026-04-16 |
| 库存超卖问题 | orders/route.ts添加悲观锁 FOR UPDATE | 2026-04-16 |
| 购物车加入时库存检查 | cart/route.ts检查库存=0时不允许加入 | 2026-04-16 |
| 购物车合并时库存检查 | cart/merge/route.ts返回失败商品列表 | 2026-04-16 |
| 错误信息多语言配置 | 创建 /src/lib/messages.ts 配置文件 | 2026-04-16 |

## 当前数据库状态

### inventory_status 表（已创建）
```sql
id | name | quantity_min | quantity_max | description
1  | 有货     | 11        | NULL     | 库存充足
2  | 库存有限  | 6         | 10       | 库存有限
3  | 库存紧张  | 1         | 5        | 库存紧张
4  | 缺货     | 0         | 0        | 缺货
...
### inventory 表结构
```sql
id | product_id | product_name | quantity | low_stock_threshold | status_id | created_at | updated_at
```

## 待开发功能（来自XPSKILL.md）

1. **优惠券系统** - 需要开发完整的优惠券领取、使用、退货流程
2. **购物车促销过期处理** - 结算时检查促销有效性
3. **结算流程** - 完整的结算API和验证
4. **订单状态管理** - 状态流转和日志
5. **支付流程** - Stripe/PayPal集成
6. **地址管理** - CRUD和默认地址
7. **运费计算** - 包邮条件和区域计费
8. **用户中心** - 个人信息、订单入口
9. **库存预警** - 低库存提醒功能
...
### 1. 错误信息多语言配置（🔴 高优先级）
**问题**：目前系统级错误信息（如"库存不足"、"支付失败"）直接写在代码里

**现状**：
- API返回固定的多语言文本，如 `{ message: "库存不足", message_en: "Insufficient stock" }`
- 这些文本写在代码里
...
**解决方案**：
- 创建 `system_messages` 表存储错误信息
- 结构：`key, zh, en, ar, description`
- 错误码在代码里，文本在数据库
- 示例：
  ```sql
  key                     | zh              | en                    | ar
  ------------------------|-----------------|----------------------|----
  error.insufficient_stock | 库存不足        | Insufficient stock    | المخزون غير كافٍ
  error.out_of_stock      | 已售罄          | Out of stock          | نفد المخزون
  ```

**涉及文件**：
- `/api/cart/route.ts` - 库存不足错误
- `/api/cart/merge/route.ts` - 库存不足错误
- `/api/orders/route.ts` - 库存不足错误

**提醒**：开发购物车、结算功能时，先按简单方案做（代码里写死），后期再迁移到数据库配置
...
### 3. JWT Secret 硬编码（🔴 安全，⚠️ 上线前必须修复）
**现状**：
- 登录时生成JWT Token使用的密钥是硬编码的默认值 `'your-secret-key'`
- 代码位置：`/src/lib/auth.ts` L4 和 `/src/app/api/auth/login/route.ts` L28, L34

**风险**：
- 如果别人知道这个默认key，可以伪造任意用户的Token
- 可以冒充管理员身份访问后台

**影响范围**：
- 前端用户登录：功能正常，但存在伪造风险
- 后台管理：当前是假登录（写死的账号密码），但如果修复后会有风险

**解决方案**：
1. 部署时设置环境变量 `JWT_SECRET=随机字符串`
2. 或者在代码里检查，没设置环境变量就报错

**修复方式**：
```typescript
// 在 auth.ts 和 login/route.ts 中
if (!process.env.JWT_SECRET) {
  throw new Error('JWT_SECRET environment variable is not set!');
}
```

**提醒**：✅ 前端登录功能完全正常，不受影响。后期部署到生产环境时，必须设置真正的JWT_SECRET。

---

### 4. 评论API权限问题（🔴 安全）
**状态**：⏳ 待修复
**位置**：`/api/reviews/route.ts`
...
**⚠️ 重要：can_stack=1 是独占！can_stack=0 是可叠加！**

| can_stack 值 | 含义 | 说明 |
|-------------|------|------|
| 1 | **独占** | 该促销为独占，不与其他促销叠加 |
| 0 | 可叠加 | 该促销可与其他促销叠加计算 |

### 有效性判断规则

1. **促销有效性判断**：
   - 条件：`product_promotions.end_time > 当前时间` 且 `product_promotions.start_time <= 当前时间`
   - 不检查 status 字段

2. **活动有效性判断**：
   - 条件：`product_activities.end_time > 当前时间` 且 `product_activities.start_time <= 当前时间`
   - 且 `activity_categories.status = 'active'`
...
SELECT
  ...
  p.price as 原价,
  pr.name as 促销名称,
  pr.discount_percent as 折扣率,
  CASE pp.can_stack WHEN 1 THEN '独占' ELSE '可叠加' END as 独占性,
  pp.end_time as 促销结束时间,
  ac.name as 活动名称,
  ac.status as 活动状态,
  pa.start_time as 活动开始时间,
  pa.end_time as 活动结束时间
...
WHERE pp.end_time > datetime('now') AND datetime(pp.start_time) <= datetime('now')
ORDER BY p.id, pp.priority;
```

### 独占促销处理逻辑

当一个商品有多个促销时：
1. 如果存在 `can_stack=1`（独占）促销 → 使用该独占促销的折扣，不叠加其他
2. 如果全部是 `can_stack=0`（可叠加）→ 按 priority 从小到大顺序叠加计算

**示例**：
- 商品18：茶杯专场(独占18%) + 紫砂壶特惠(可叠加20%)
  → 结果：使用茶杯专场18%折扣，紫砂壶特惠被忽略
- 商品19：茶杯专场(可叠加18%) + 紫砂壶特惠(独占20%)
  → 结果：使用紫砂壶特惠20%折扣，茶杯专场被忽略
